...
def get_url_data_in_ranking_list(typeId, movie_count, rating, vote_count):
    ...
    try:
        context = ssl._create_unverified_context()  # 屏蔽ssl证书
...
def get_url_data_in_keyWord(key_word):
    ...
    wait = None
    try:
        browser = webdriver.Chrome(executable_path='C:/Program Files/Google/Chrome/Application/chromedriver/chromedriver.exe', chrome_options=chrome_options)  # 设置chromedriver驱动路径
